      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <template v-for="(s, i) in steps" :key="i">
              <div class="flex items-center gap-2 cursor-pointer" @click="i < step ? step = i : null">
                <div :class="['w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all',
                  i < step ? 'bg-teal-500 text-white' : i === step ? 'bg-teal-100 text-teal-700 ring-2 ring-teal-400' : 'bg-gray-100 text-gray-400']">
              ...
              <div v-if="i < steps.length - 1" class="w-12 h-px mx-2" :class="i < step ? 'bg-teal-400' : 'bg-gray-200'"></div>
            </template>
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-8 py-6">
          ...
          <div v-if="step === 0" class="space-y-4">
            ...
            <div v-if="newProject">
              <label class="block text-xs font-medium text-gray-600 mb-1.5">שם פרויקט *</label>
              <input v-model="form.project_name" type="text" placeholder="שם הפרויקט"
                ...
                :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition', fc('project_name')]" />
              <span v-if="fe.project_name" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.project_name }}</span>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="form.priority_id" type="text" placeholder="P-1001"
                  ...
                  :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-teal-300 transition', fc('priority_id')]" />
                <span v-if="fe.priority_id" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.priority_id }}</span>
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5">תאריך התחלה *</label>
                <DatePicker v-model="form.start_date"
                  :input-class="'w-full bg-gray-50 border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition ' + (fe.start_date ? 'border-red-400 bg-red-50' : 'border-gray-200')" />
                <span v-if="fe.start_date" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.start_date }}</span>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model="form.manager" type="text" placeholder="שם מלא"
                  ...
                  :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition', fc('manager')]" />
                <span v-if="fe.manager" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.manager }}</span>
              </div>
              <div>
                ...
                <input v-model="form.client" type="text" placeholder="שם החברה/לקוח"
                  ...
                  :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition', fc('client')]" />
                <span v-if="fe.client" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.client }}</span>
              </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5">תחום</label>
                <select v-model="form.area" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition">
              ...
              <div>
                ...
                <select v-model="form.axis" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition">
            ...
            <div>
              ...
              <textarea v-model="form.description" rows="3" placeholder="תיאור כללי של הפרויקט..."
                ...
                :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition resize-none', fc('description')]"></textarea>
              <div class="flex justify-between mt-0.5">
                <span v-if="fe.description" class="text-red-500 text-[10px]">{{ fe.description }}</span>
                <span v-else></span>
                <span class="text-gray-400 text-[10px]">{{ (form.description || '').length }} תווים</span>
              </div>
            </div>
          </div>
          ...
          <div v-if="step === 1" class="space-y-5">
            <div>
              <label class="block text-xs font-medium text-gray-600 mb-1.5">סך הכנסות הפרויקט (₪) *</label>
              <input v-model.number="form.total_revenue" type="number" placeholder="0" min="1"
                ...
                :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition', fc('total_revenue')]" />
              <span v-if="fe.total_revenue" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.total_revenue }}</span>
            </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs font-bold" :class="paymentTermsTotal === 100 ? 'text-teal-600' : 'text-orange-500'">
                  סה"כ: {{ paymentTermsTotal }}%
                </span>
              </div>
              <div class="space-y-2">
                <div v-for="(term, i) in form.revenue_payment_terms" :key="i"
                  class="flex items-center gap-2 bg-gray-50 rounded-xl border border-gray-200 px-3 py-2">
                  <select v-model="term.type" class="flex-1 bg-white border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300">
                  ...
                  <div class="flex items-center gap-1">
                    <input v-model.number="term.percent" type="number" min="0" max="100" placeholder="0"
                      class="w-16 bg-white border border-gray-200 rounded-lg px-2 py-2 text-xs text-center focus:outline-none focus:ring-1 focus:ring-teal-300" />
                    <span class="text-xs text-gray-400">%</span>
                  </div>
                  <div v-if="form.total_revenue && term.percent" class="text-[10px] text-gray-400 min-w-[60px] text-left">
                ...
                <button @click="form.revenue_payment_terms.push({ type: 'שוטף+30', percent: 0 })"
                  class="w-full py-2 border-2 border-dashed border-gray-300 rounded-xl text-xs text-gray-500 hover:bg-gray-50 transition flex items-center justify-center gap-1">
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs font-bold" :class="forecastTotal === 100 ? 'text-teal-600' : 'text-orange-500'">
                  סה"כ: {{ forecastTotal }}%
                </span>
              </div>
              <div class="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                <table class="w-full text-xs">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td v-for="m in 12" :key="m" class="px-1 py-1.5 text-center">
                        <input v-model.number="form.revenue_forecast[m]" @input="clampForecast(m)" type="number" min="0" max="100" placeholder="0"
                          class="w-full bg-white border border-gray-200 rounded-lg px-1 py-1.5 text-center text-xs focus:outline-none focus:ring-1 focus:ring-teal-300 transition" />
                      </td>
                    </tr>
                    ...
                    <tr class="bg-amber-50/50 border-t border-gray-200">
                      <td v-for="m in 12" :key="m" class="px-1 py-1.5 text-center text-[10px] text-amber-700 font-medium">
                ...
                <!-- Legend -->
                <div class="flex items-center gap-4 px-3 py-2 border-t border-gray-200 text-[10px] text-gray-400">
                  <div class="flex items-center gap-1"><span class="w-2 h-2 rounded bg-teal-200"></span> סכום צפוי</div>
          ...
          <div v-if="step === 2" class="space-y-5">
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input v-model.number="form.total_budget" type="number" placeholder="0" min="1"
                  ...
                  :class="['w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition', fc('total_budget')]" />
                <span v-if="fe.total_budget" class="text-red-500 text-[10px] mt-0.5 block">{{ fe.total_budget }}</span>
              </div>
              <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5">תנאי תשלום הוצאות</label>
                <div class="flex gap-2">
                  <select v-model="form.payment_terms_expense.type" class="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition">
                    <option value="מזומן">מזומן</option>
                    <option value="מקדמה">מקדמה</option>
                    <option value="שוטף+30">שוטף+30</option>
                    <option value="שוטף+45">שוטף+45</option>
                  ...
                  <input v-model="form.payment_terms_expense.notes" type="text" placeholder="הערות"
                    class="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 transition" />
                </div>
              </div>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="border border-gray-200 rounded-xl overflow-hidden">
                <button @click="toggleCategory('subcontractors')"
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    קבלני משנה
                    <span v-if="form.subcontractors.length" class="text-xs bg-teal-100 text-teal-600 px-2 py-0.5 rounded-full">{{ form.subcontractors.length }}</span>
                  </div>
                ...
                <div v-if="activeCategory === 'subcontractors'" class="p-4 space-y-3">
                  <div v-for="(sub, i) in form.subcontractors" :key="i"
                    ...
                    <div class="grid grid-cols-2 gap-2">
                      <div>
                        <label class="block text-[10px] text-gray-400 mb-1">שם קבלן</label>
                        <input v-model="sub.name" type="text" placeholder="שם הקבלן"
                          @blur="vf(`sub_name_${i}`, sub.name, { required: true, minLen: 2 })"
                          :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`sub_name_${i}`)]" />
                        <span v-if="fe[`sub_name_${i}`]" class="text-red-500 text-[10px]">{{ fe[`sub_name_${i}`] }}</span>
                      </div>
                      <div>
                        <label class="block text-[10px] text-gray-400 mb-1">התחייבות לספק (ש"ח)</label>
                        <input v-model.number="sub.monthly_amount" type="number" placeholder="0" min="1"
                          ...
                          :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`sub_amount_${i}`)]" />
                        <span v-if="fe[`sub_amount_${i}`]" class="text-red-500 text-[10px]">{{ fe[`sub_amount_${i}`] }}</span>
                      </div>
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                      <div>
                        <label class="block text-[10px] text-gray-400 mb-1">תאריך התחלה</label>
                        <DatePicker v-model="sub.start_date"
                          input-class="w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300" />
                      </div>
                      <div>
                        <label class="block text-[10px] text-gray-400 mb-1">תאריך סיום</label>
                        <DatePicker v-model="sub.end_date"
                          input-class="w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300" />
                      </div>
                      <div>
                        <label class="block text-[10px] text-gray-400 mb-1">תנאי תשלום</label>
                        <select v-model="sub.payment_terms"
                          class="w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300">
                          <option value="מזומן">מזומן</option>
                          <option value="מקדמה">מקדמה</option>
                          <option value="שוטף+30">שוטף+30</option>
                          <option value="שוטף+45">שוטף+45</option>
                  ...
                  <button @click="addSubcontractor"
                    class="w-full py-2.5 border-2 border-dashed border-teal-300 rounded-xl text-sm text-teal-500 font-medium hover:bg-teal-50 transition flex items-center justify-center gap-2">
                ...
                <div v-if="activeCategory === 'manpower'" class="p-4 space-y-3">
                  ...
                  <div class="space-y-3">
                    <div v-for="(line, i) in form.expense_lines_manpower" :key="i"
                      class="bg-gray-50 rounded-xl border border-gray-200 p-3 space-y-2 hover:border-gray-300 transition-colors">
                      <div class="flex items-center justify-between mb-1">
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        <div>
                          <label class="block text-[10px] text-gray-400 mb-1">פירוט</label>
                          <input v-model="line.name" type="text" placeholder="תיאור ההוצאה"
                            @blur="vf(`mp_name_${i}`, line.name, { required: true, minLen: 2 })"
                            :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`mp_name_${i}`)]" />
                          <span v-if="fe[`mp_name_${i}`]" class="text-red-500 text-[10px]">{{ fe[`mp_name_${i}`] }}</span>
                        </div>
                        <div>
                          <label class="block text-[10px] text-gray-400 mb-1">התחייבות לספק (₪)</label>
                          <input v-model.number="line.monthly_amount" type="number" placeholder="0" min="1"
                            ...
                            :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`mp_amount_${i}`)]" />
                          <span v-if="fe[`mp_amount_${i}`]" class="text-red-500 text-[10px]">{{ fe[`mp_amount_${i}`] }}</span>
                        </div>
                      </div>
                      <div class="grid grid-cols-2 gap-2">
                        <div>
                          <label class="block text-[10px] text-gray-400 mb-1">מחודש</label>
                          <select v-model.number="line.start_month" class="w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300">
                            <option v-for="m in 12" :key="m" :value="m">חודש {{ m }}</option>
                          </select>
                        </div>
                        <div>
                          <label class="block text-[10px] text-gray-400 mb-1">עד חודש</label>
                          <select v-model.number="line.end_month"
                            :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`mp_end_${i}`)]">
                            <option v-for="m in 12" :key="m" :value="m">חודש {{ m }}</option>
                          </select>
                          <span v-if="fe[`mp_end_${i}`]" class="text-red-500 text-[10px]">{{ fe[`mp_end_${i}`] }}</span>
                        </div>
                      </div>
                      <!-- Line total preview -->
                      <div v-if="line.monthly_amount && line.start_month && line.end_month" class="bg-white rounded-lg px-2 py-1.5 text-[10px] text-gray-400 flex justify-between">
                  ...
                  <button @click="addExpenseLine('manpower')"
                    class="w-full py-2.5 border-2 border-dashed border-gray-300 rounded-xl text-xs text-gray-500 font-medium hover:bg-gray-50 hover:border-gray-400 transition flex items-center justify-center gap-1.5">
              ...
              <div v-for="cat in standardCategories" :key="cat.key" class="border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div v-show="activeCategory === cat.key" class="accordion-body">
                  <div class="p-4 space-y-3">
                    ...
                    <div v-if="!form['expense_lines_' + cat.key]?.length" class="text-center py-6">
                      ...
                      <div class="text-[10px] text-gray-300 mt-0.5">לחץ למטה להוספת ספק</div>
                    </div>
                    <!-- Supplier cards -->
                    <div v-for="(line, i) in form['expense_lines_' + cat.key]" :key="i"
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        <div>
                          <label class="block text-[10px] text-gray-400 mb-1">שם ספק</label>
                          <input v-model="line.name" type="text" placeholder="שם הספק"
                            @blur="vf(`${cat.key}_name_${i}`, line.name, { required: true, minLen: 2 })"
                            :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`${cat.key}_name_${i}`)]" />
                          <span v-if="fe[`${cat.key}_name_${i}`]" class="text-red-500 text-[10px]">{{ fe[`${cat.key}_name_${i}`] }}</span>
                        </div>
                        <div>
                          <label class="block text-[10px] text-gray-400 mb-1">התחייבות לספק (₪)</label>
                          <input v-model.number="line.monthly_amount" type="number" placeholder="0" min="1"
                            @blur="vf(`${cat.key}_amount_${i}`, line.monthly_amount, { required: true, positive: true })"
                            :class="['w-full bg-white border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-teal-300', fc(`${cat.key}_amount_${i}`)]" />
                          <span v-if="fe[`${cat.key}_amount_${i}`]" class="text-red-500 text-[10px]">{{ fe[`${cat.key}_amount_${i}`] }}</span>
                        </div>
                      </div>
                      <div class="grid grid-cols-3 gap-2">
                        <div>
            ...
            <div v-if="form.total_budget" class="space-y-1.5">
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs font-bold" :class="budgetUtilization > 100 ? 'text-red-500' : budgetUtilization > 80 ? 'text-orange-500' : 'text-teal-600'">
                  {{ budgetUtilization.toFixed(0) }}%
                </span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                <div class="h-full rounded-full transition-all duration-500"
                  :class="budgetUtilization > 100 ? 'bg-red-500' : budgetUtilization > 80 ? 'bg-orange-400' : 'bg-teal-500'"
            ...
            <div class="bg-gray-50 rounded-xl border border-gray-200 px-4 py-3">
              ...
              <div v-if="form.total_budget" class="flex items-center justify-between mt-1">
                <span class="text-[10px] text-gray-400">תקציב מאושר:</span>
                <span class="text-[10px] text-gray-400">{{ form.total_budget.toLocaleString('he-IL') }} ₪</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="px-8 py-4 border-t border-gray-100 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            ...
            <button v-if="step < steps.length - 1" @click="nextStep"
              class="px-5 py-2.5 bg-teal-500 text-white text-sm font-medium rounded-xl hover:bg-teal-600 transition flex items-center gap-1">
              הבא
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
            </button>
            <button v-else @click="save" :disabled="saving"
              class="px-6 py-2.5 bg-teal-500 text-white text-sm font-medium rounded-xl hover:bg-teal-600 disabled:opacity-50 transition flex items-center gap-2">
