<template>
  <div>
      ...
      <div class="flex items-start justify-between mb-8">
        <div>
          <div class="flex items-center gap-3 mb-1">
            ...
            <span v-if="pnlData.meta?.priority_id"
              class="bg-blue-50 text-blue-600 text-xs font-mono font-semibold px-2.5 py-1 rounded-lg border border-blue-100">
              {{ pnlData.meta.priority_id }}
            </span>
          </div>
          <p class="text-sm text-gray-400">דוח רווח והפסד חודשי</p>
        </div>
        <div class="flex items-center gap-2">
          ...
          <button @click="showReportModal = true"
            class="px-4 py-2.5 bg-lime-500 text-white text-sm font-medium rounded-xl hover:bg-lime-600 transition flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            <div v-for="r in reports" :key="r.id" class="px-6 py-3 flex items-start gap-3">
              <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5"
                :class="{
                  ...
                  'bg-blue-50 text-blue-500': r.type === 'note'
                }">
        ...
        <div class="bg-white rounded-2xl px-5 py-3 shadow-sm border border-gray-100 flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <div>
              ...
              <div class="text-sm font-semibold font-mono text-blue-600">{{ pnlData.meta?.priority_id || '-' }}</div>
            </div>
          </div>
          <div class="w-px h-8 bg-gray-200"></div>
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 bg-lime-100 rounded-lg flex items-center justify-center">
              <svg class="w-4 h-4 text-lime-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <div class="text-2xl font-bold"
            :class="pnlData.summary.margin != null && pnlData.summary.margin >= 20 ? 'text-lime-600' : 'text-orange-500'">
          ...
          <div class="mt-2 bg-gray-100 rounded-full h-1.5 overflow-hidden">
            <div class="h-full rounded-full transition-all duration-500"
              :class="pnlData.summary.margin >= 20 ? 'bg-lime-400' : 'bg-orange-400'"
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <template v-for="m in pnlData.months" :key="m.month">
                ...
                <tr @click="toggleDrilldown(m.month)"
                  ...
                  <td class="px-4 py-3 text-gray-600 group-hover:text-blue-600 group-hover:underline">{{ fmt(m.op_expenses) }}</td>
                  <td class="px-4 py-3" :class="m.gross_profit >= 0 ? 'text-green-600' : 'text-red-500'">{{ fmt(m.gross_profit) }}</td>
                  <td class="px-4 py-3 text-gray-600 group-hover:text-blue-600 group-hover:underline">{{ fmt(m.salary_expenses) }}</td>
                  ...
                  <td class="px-4 py-3">
                    <span v-if="m.margin != null"
                      ...
                      :class="m.margin_alert ? 'bg-red-50 text-red-600' : 'bg-lime-50 text-lime-700'">
                      {{ m.margin }}%
                    </span>
                ...
                <!-- Drill-down row -->
                <tr v-if="expandedMonth === m.month" class="bg-blue-50/30">
            ...
            <tfoot>
              <tr class="bg-lime-50/50 font-bold border-t-2 border-lime-200">
                ...
                <td class="px-4 py-4">
                  <span v-if="pnlData.summary.margin != null" class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-lime-100 text-lime-700">
    ...
    <ProjectFormModal :show="showProjectForm" :project="selectedProject" @close="showProjectForm = false" @saved="loadPnl" />
  </div>
</template>

<script setup>
import { ref, computed, onMounted, watch } from 'vue'
import { getProjects, getPnl, getProjectCashflow, getReports, formatNumber } from '../services/api'
