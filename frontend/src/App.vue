<template>
  ...
  <div v-else class="min-h-screen" dir="rtl" style="background: var(--bg);">
        ...
        <div class="flex items-center h-14 sm:h-16 gap-3 sm:gap-6">
          ...
          <button @click="activeTab = 'dashboard'; selectedProject = ''"
            ...
            <div class="hidden sm:block">
              <div class="text-sm font-bold text-gray-800 leading-tight">Logfi</div>
          ...
          <div class="flex items-center gap-2 sm:gap-3 flex-shrink-0 mr-auto">
            <!-- New project dropdown (hidden on mobile - available in hamburger menu) -->
            <div class="hidden md:block relative">
              <button @click="showProjectDropdown = !showProjectDropdown"
                class="flex items-center gap-1.5 px-3.5 py-2 bg-gray-900 text-white rounded-xl text-sm font-medium hover:bg-gray-800 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
                </svg>
                <span class="hidden lg:inline">פרויקט חדש</span>
                <svg class="w-3 h-3 hidden lg:block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
              </button>
              <div v-if="showProjectDropdown" class="fixed inset-0 z-40" @click="showProjectDropdown = false"></div>
              <div v-if="showProjectDropdown" class="absolute left-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 min-w-[180px] z-50">
                <button @click="showNewProjectModal = true; showProjectDropdown = false"
                  class="w-full text-right px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                  פרויקט
                </button>
                <button @click="showRecurringToast = true; showProjectDropdown = false; setTimeout(() => showRecurringToast = false, 3000)"
                  class="w-full text-right px-4 py-2.5 text-sm text-gray-400 hover:bg-gray-50 transition-colors">
                  פרויקט מחזורי
                </button>
              </div>
            </div>
            ...
            <!-- User menu (hidden on mobile - available in hamburger menu) -->
            <div class="hidden md:block relative pr-3 border-r border-gray-200">
              <button @click="showUserMenu = !showUserMenu" class="flex items-center gap-2.5 hover:bg-gray-50 rounded-xl px-2 py-1.5 transition-colors">
                <div class="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center overflow-hidden">
                  <img v-if="userProfile.avatar" :src="userProfile.avatar" class="w-full h-full object-cover" />
                  <span v-else class="text-emerald-800 font-bold text-xs">{{ userInitials }}</span>
                </div>
                <div class="hidden lg:block text-right">
                  <div class="text-sm font-semibold text-gray-700 leading-tight">{{ userDisplayName }}</div>
                  <div class="text-[11px] text-gray-400">{{ userProfile.role || 'Logfi' }}</div>
                </div>
                <svg class="w-3.5 h-3.5 text-gray-400 hidden lg:block" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
              </button>
              <div v-if="showUserMenu" class="fixed inset-0 z-40" @click="showUserMenu = false"></div>
              <div v-if="showUserMenu" class="absolute left-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1.5 min-w-[200px] z-50" dir="rtl">
                <button @click="showUserProfile = true; showUserMenu = false" class="w-full text-right px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors flex items-center gap-3">
                  <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                  פרטי משתמש
                </button>
                <button @click="showSettings = true; showUserMenu = false" class="w-full text-right px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors flex items-center gap-3">
                ...
                <button @click="logout(); showUserMenu = false" class="w-full text-right px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors flex items-center gap-3">
    ...
    <transition name="slide-down">
      <div v-if="mobileMenuOpen" class="fixed inset-0 top-14 z-20 bg-white md:hidden overflow-y-auto" dir="rtl">
        <div class="p-4 space-y-1">
          ...
          <button @click="showRecurringToast = true; mobileMenuOpen = false; setTimeout(() => showRecurringToast = false, 3000)"
            class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-400 border border-gray-200">
    ...
    <!-- Recurring project toast -->
    <Teleport to="body">
      <transition name="fade">
        <div v-if="showRecurringToast" class="fixed top-6 left-1/2 -translate-x-1/2 z-[100] bg-gray-900 text-white px-6 py-3 rounded-xl shadow-lg text-sm font-medium" dir="rtl">
    ...
    <Teleport to="body">
      <div v-if="showRobotMsg" class="fixed inset-0 z-[100] flex items-center justify-center" dir="rtl">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-sm mx-4 text-center transform animate-bounce-in">
          ...
          <div class="text-xl font-bold text-gray-800 mb-2">איזה כיף להיות הLogfi!</div>
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-3">
            <div class="flex gap-2">
              ...
              <div class="bg-gray-50 rounded-xl rounded-tr-none px-3 py-2 text-sm text-gray-700 max-w-[85%]">
                היי! אני העוזר החכם של Logfi. אפשר לשאול אותי שאלות על הנתונים הפיננסיים, פרויקטים, ותזרים מזומנים.
              </div>
            </div>
          </div>
          <!-- Chat input -->
...
const userDisplayName = computed(() => userProfile.value.full_name || userProfile.value.role || userProfile.value.username || 'משתמש')
